...
description: Design or critique photographic and cinematic aesthetics for AI image and video prompts using lighting, framing, lenses, tone, color, texture, film styles, motion, and model-specific prompt dialects. Use for visual-direction prompts, photo analysis, and image-edit instructions; do not use to call generation APIs, build brand systems, or create data visualizations.
---
...
| 3 相機高度 | 觀眾與主體的權力關係 | 17, 33, 39（＋眼平＝預設，不用寫） |
...
使用者沒說就問，或預設用自然語言方言並在回覆中註明假設。細節見 `references/09-model-dialects.md`。
...
5. **照固定順序組裝**（順序即權重，越前面影響越大）：
   ...
   主體 → 景別 → 鏡頭焦距 → 視角/朝向 → 光位 → 光質 → 光源與色溫
        → 影調與對比 → 色彩 → 景深 → 質感/載體 → 風格總結 → 畫幅（句尾或參數區）
   ```
...
先讀 `references/08-motion.md`。影片提示詞必須描述**變化**（開場是 X、過程中 Y、結束在 Z），
純靜態描述會得到幾乎不動的片段。一個片段 = 1 個攝影機運動 + 1 個主體動作，
且**單一片段內禁止寫 cut / transition / 然後切到**，轉場一律後製、多鏡頭分次生成。
...
6. **給數字**。「側光」不如 `90 degree side key light`，「淺景深」不如 `85mm at f/1.4`。
7. **描述陰影，不只描述光**。模型判斷光位主要靠陰影落點，寫清楚暗部在哪。
8. **畫幅是必填的**。不給就是預設 1:1，「電影感」再怎麼堆都不會有。`2.39:1` 是最便宜的電影感來源。
...
| `references/03-framing.md` | 景別/景深/動態 9 項、視角/構圖 9 項 + 鏡頭第 10 軸 + 構圖補充 10 項 + 多人構圖 | 要決定拍多近、用什麼焦段、從哪拍時 |
...
| `references/08-motion.md` | 攝影機運動、主體動作、片長與轉場規則 | 產生影片提示詞時 |
