...
> **未指定 runtime 的邊界**：When the target model or provider is unknown, return a provider-neutral natural-language
> prompt. Do not name or list compatible models, claim cross-model reliability, recommend a negative-prompt field,
> suggest provider parameter syntax, or use the model table and model-specific output formats below. Those sections apply
> only after the user selects the runtime.
...
7. **畫幅只在使用者或已選 runtime 確認時給，而且只給一次。** 平台有欄位就用欄位，提示詞裡不要重複寫；
   純文字介面才寫進句尾。未指定就省略，不用美學慣例代填。
...
13. **已選 runtime 的交付要能被重現**：模型名稱、完整提示詞、negative（若有）、畫幅、seed（若有）一起記下來。
    少記畫幅是最常見的失誤 —— 畫幅一改，景別與構圖比例全部重算，等於整張重來，
    它是唯一一個改了就沒辦法再用「只改一個變數」來比較的項目。
14. **強調要寫成一句普通的話，不要寫成標籤。** `CRITICAL:`、`IMPORTANT:`、`MUST:`、連續全大寫的字串，
    在「把像參數或標籤的字串畫進畫面」的模型上與參數字串同類（見第 8 條，以及對照表 GPT Image 2 那一列）。
    第 8 條講的是有文件可查的參數尾綴，**全大寫標籤是那一條的外推**，會不會真的被畫出來依模型而異；
    但標籤形態並沒有換到任何額外權重，只多了一個把字畫進畫面的風險。
    要強調就用提示詞本身的語言寫成正常的句子（`Most important: ...`），
    並依第 2 條的分流決定它擺在哪裡：有 negative 欄位就不要在正文放強調句；
    指令遵循型模型照第 2 條把否定集中在正文末尾；其餘情況照 `SKILL.md` 硬規則 15 貼在被守護的那句之後。
...
直接輸出 provider-neutral 的自然語言視覺描述。不要列模型名稱、相容性、negative 欄位、參數尾綴、
seed 或平台操作建議；使用者已指定的畫幅可保留，其他 runtime 欄位全部省略。只有方言差異會實質改變
使用者所求的 artifact 時，才把「目標模型是什麼？」列為一個最小問題。
...
只保留使用者指定或已選 runtime 明確要求的畫幅。風格參考中的歷史畫幅只能作為使用者主動詢問時的
美學選項，不得覆寫 delivery 規格，也不得在需求未指定時補預設值。
