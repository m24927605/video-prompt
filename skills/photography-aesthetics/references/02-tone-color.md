...
**點名要涵蓋不是物件的顏色載體。** 只點名陳設物（牆、椅、衣）會漏掉四類同樣把顏色帶進畫面的東西：反射面（玻璃、水面、金屬、濕地）、鏡面高光、散景光點，以及**光源本身**。它們不在美術陳設的清單裡，所以最常是第四個色相回來的路徑——而且它們通常是畫面裡最亮的像素，一出現就贏過所有陳設。

做法仍然是正面列舉，不是加否定句（理由見上一段）：`reflections in the glass and the wet floor drawn from the same three, the practical lamps themselves reading warm off-white, bokeh discs staying inside the palette`。

同一條也適用於下一條的重點色：重點色的「全畫面只出現一次」要連它的反射與高光一起算，反射算第二次。
...
**影片情境：三條件是逐鏡各自成立，不是整部片成立一次。** 每一個鏡次各自只准一個重點色；
某一鏡確實需要兩個時，必須在**該鏡**明寫是哪兩個、各自掛在哪個表面，沒寫就等於沒有上限。
跨鏡的重點色分配屬於 color script，由 `seedance-film-producer` 擁有；本檔只負責單一畫面內的三條件。
...
- **不照做會發生什麼**：抽象色名沒有告訴模型顏色該落在哪，最省力的解法就是整張蒙一層色——
  ...
  兩個座標都不給，模型只能全域套用。
- **顏色載體不只有陳設物**：反射面、鏡面高光、散景光點與光源本身同樣承載色相，點名時要一起算
  （載體清單見〈限制調色盤〉）。
...
#### 規則九｜要跨張維持同一個顏色時，給它一個可定址的數值錨點

- **做法**：寫成「材質類別 + 數值色標」，不要只寫色名——`the wall paint pinned to one hex value, #RRGGBB`、
  `the coat fabric pinned to #RRGGBB`（填上實際值）。數值仍必須依規則七掛在具體表面上，不可單獨浮在句子裡。
- **它換來什麼**：色名是一個區間，每次重生都在區間裡重抽；數值是一個點。你可以在下一張沿用同一個字串
  （照 `SKILL.md` 硬規則 14 的配套，逐字複製而不是重打），也可以在檢查時明確說出「這次偏了」。
  它買到的是**可重複與可檢查**，不是命中率。
- **界線**：數值是創意目標，不是 runtime 事實（`SKILL.md` 硬規則 6）。模型不保證命中，
  而且色標還會被載體、影調與白平衡各搬一次；使用者明確限縮的 video visual-look subcontract
  不得自行補進任何數值。
- **不要對膚色下數值。** 膚色走規則二的 `preserve natural skin tone`；把膚色釘成一個數值只會得到色塊。
- **把數值釘在靜圖那一端。** 色標是寫給一次靜態生成看的；用一張已定色的靜圖去驅動影片時，
  不要預設影片端會沿用同一個數值——它可能依自己的先驗重新落值，鏡次之間也不保證一致
  （這是使用觀察，不是任何平台的已驗證行為）。保守做法是把數值釘在**會被當成輸入的那張靜圖與關鍵幀**上，
  影片端只寫色名與載體（規則七）。這不是保險失效，是責任轉移——影片端的一致性由輸入圖守，不由文字守。
  跨鏡要落哪幾個重點色，仍是 color script 的決定。
