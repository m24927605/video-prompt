...
| 構圖無聊、像商品目錄 | 只有景別沒有視角；軸 A 未指定就是未約束，模型自行決定機位高低，常落在眼平正面 | T39 低角度視角 或 T17 高角度拍攝 或 T26 斜側視角，任選一個；或 T34 封閉構圖給畫面加框 | T35 居中構圖（若不是刻意要對稱）、未經指定而落在眼平正面的機位 |
...
| 3 | 相機高度 | T33 鳥瞰（俯 75–90°）↔ T17 高角度拍攝（俯 15–45°）↔ 眼平（要用就明寫，見 `03-framing.md` 軸 A）↔ T39 低角度視角（仰 15–45°） | 主體相對觀眾只有一個權力值：垂直維度塌陷、人物去人格化成色塊 → T33；觀察／敘事性俯視、仍讀得到五官 → T17；平等／紀實 → 眼平（明寫高度基準與 0° 傾角，不要靠省略）；威嚴／體量／英雄 → T39 | 無。俯 45–75° 與仰 45–75° 是兩檔之間的過渡帶，72 項裡沒有編號，要用就直接寫死度數，不要疊兩個編號 |
